...
def Bellman_Ford(start, end1, INF=99999):
    # 初始化源点与所有点之间的最短距离
...
car_list.sort(key=lambda x:(x.maxspeed),reverse=True)#-x.planTime/100
#让快的先走
...
for i in range(len(car_list)):
    ...
    car_list[i].route = Bellman_Ford(start, end)
    ...
    if i > 2000 - 1:
        for p in car_list[i-2000].route:
            for j in range(len(road_list)):
                if p == road_list[j].ID:
                    road_list[j].congestion -= 1
                    break
